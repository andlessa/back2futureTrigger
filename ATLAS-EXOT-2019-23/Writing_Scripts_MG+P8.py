...
masses_phi = [1000,1000]
masses_S = [275,475]
nevent = [10000,10000]
ufoDir = os.path.abspath('./UFOs')
mg5Dir = os.path.abspath('./MG5')

fnames = []
for mPhi,mS,nevt in zip(masses_phi,masses_S,nevent):
    fname = f"script_mH{mPhi}_mS{mS}.txt"
    f = open(fname, 'w') # Creation of the Script to follow for MG.
    f.write(f"import model {ufoDir}/HAHM_MG5model_v3/HAHM_variableMW_v3_UFO \n") # Import the model.
...
for fname in fnames:
    ...
    out = subprocess.check_output(f"./bin/mg5_aMC -f {fname}",
                                  shell=True,text=True,cwd=mg5Dir)
