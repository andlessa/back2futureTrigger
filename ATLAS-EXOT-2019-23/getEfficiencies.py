...
class FileObjWrapper(object):
    ...
    def __init__(self,inputFile=None):
        ...
        # Guess file type
        if self.inputFile:
            if 'hepmc' in self.inputFile.lower():
                self.fileType = 'hepmc'
            elif 'root' in self.inputFile.lower():
                self.fileType = 'root'
            else:
                logger.error(f'Unknown file type for {self.inputFile}')
    ...
    def loadFile(self):
        ...
        elif self.fileType == 'hepmc':
            fileObj = pyhepmc.open(inputFile) # Open HEPMC file
        elif self.fileType == 'root':
            f = ROOT.TFile(inputFile,'read')
            fileObj = f.Get("Delphes")
    ...
    def next(self):
        ...
        if self.fileObj is None:
            ...
            event = self.fileObj.read()
            if event is not None:
                self.entry += 1
                return event
        elif self.fileType == 'root':
            ebytes = self.fileObj.GetEntry(self.entry)
            if ebytes > 0:
                self.entry += 1
                return self.fileObj
            
        # If getting the next event failed, return None
        return None

    def close(self):

        if self.fileType == 'hepmc':
            self.fileObj.close()
        elif self.fileType == 'root':
            self.fileObj.Close()


def getDataFromTree(tree,llps,invisibles):
...
def getDataFromEvent(event,llps,invisibles):    

def getDataFrom(event,llps,invisibles):


        visible_particles,invisible_particles,llp_particles =  getDataFromTree(tree,llps,invisibles)
        eventDict = {}
...
def getEfficiencies(inputFile,tauList,
    ...
    # Load File
    fileObj = FileObjWrapper(inputFile)
    if fileObj is None:
        return None
...
if __name__ == "__main__":
    ...
    ap.add_argument('-f', '--inputfile',nargs='+',
            help='path to the input (HepMC or ROOT) file(s).')
    ...
    if any('.root' in inputFile for inputFile in inputFileList):
